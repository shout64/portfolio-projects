...
**main.py** is a script that connects to the production database and calls two stored procedures, commits the transactions and gets the results of the query. Then it generates a CSV file with today's date for each query, uses the Paramiko library to connect to the finance system's SFTP folder and transfer the files. Lastly, the script looks at the dates in the file names and deletes any files older than the specified number of days.
