SELECT DISTINCT
	...
	(SELECT STRING_AGG(st.SPORTS_CDE_DESCRIPTION, ', ')
		...
		WHERE st.ID_NUM = vas.id_num
		AND (CAST(st.YR_CDE as varchar) + st.TRM_CDE) = csm.SESS_CDE
		GROUP BY st.id_num
	) as Sports
FROM VW_ACTIVE_STUDENTS vas
	LEFT OUTER JOIN Name_Master nm	       ON vas.id_num = nm.ID_NUM
	INNER JOIN	STUDENT_DIV_MAST sdm   ON vas.id_num = sdm.ID_NUM
	INNER JOIN	STUDENT_MASTER sm      ON vas.id_num = sm.ID_NUM
	...
	LEFT OUTER JOIN STUD_TERM_SUM_DIV stsd ON vas.id_num = stsd.ID_NUM AND (CAST(stsd.YR_CDE as varchar) + stsd.TRM_CDE) = csm.SESS_CDE
	LEFT OUTER JOIN SPORTS_TRACKING_V st   ON nm.ID_NUM = st.ID_NUM AND vas.id_num = st.ID_NUM AND (CAST(st.YR_CDE as varchar) + st.TRM_CDE) = csm.SESS_CDE
WHERE 
	stsd.DIV_CDE = 'UG'
