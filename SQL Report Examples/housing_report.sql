...
FROM VW_ACTIVE_STUDENTS vas
	...
	LEFT OUTER JOIN STUD_TERM_SUM_DIV stsd ON vas.id_num = stsd.ID_NUM
		AND (CAST(stsd.YR_CDE as varchar) + stsd.TRM_CDE) = csm.SESS_CDE
	LEFT OUTER JOIN SPORTS_TRACKING_V st   ON nm.ID_NUM = st.ID_NUM
		AND vas.id_num = st.ID_NUM
		AND (CAST(st.YR_CDE as varchar) + st.TRM_CDE) = csm.SESS_CDE
WHERE 
	stsd.DIV_CDE = 'UG'
