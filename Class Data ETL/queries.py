...
WHERE
	...
	END = sch.YR_CDE
	AND CASE
		WHEN MONTH(GETDATE()) >= 8 THEN 'FA'
		WHEN MONTH(GETDATE()) >= 6
			AND MONTH(GETDATE()) < 8 THEN 'SU'
		ELSE 'SP'
	END = sch.TRM_CDE
...
WHERE
    ...
    AND CASE
            WHEN MONTH(GETDATE()) >= 8 THEN 'FA'
            WHEN MONTH(GETDATE()) >= 6 AND MONTH(GETDATE()) < 8 THEN 'SU'
            ELSE 'SP'
        END = ftl.TRM_CDE
...
WHERE
	CASE
		...
		ELSE YEAR(GETDATE()) - 1
	END = YR_CDE
	--can remove this section if they want the whole year
	AND CASE
		WHEN MONTH(GETDATE()) >= 8 THEN 'FA'
		WHEN MONTH(GETDATE()) >= 6 AND MONTH(GETDATE()) < 8 THEN 'SU'
		ELSE 'SP'
